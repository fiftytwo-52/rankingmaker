...
**This folder is local-only.** It was pushed to GitHub once so the structure exists in history, then it was
untracked (`git rm -r --cached Tracker`) and added to `.gitignore`. That means:

- Editing the workbook never creates a change that can be pushed — your upload list stays private.
- The copy on GitHub is frozen at the first push; it is **not** updated by later commits.
...
later. Use these templates so every short is instantly recognisable by group:

| Group | Title template | Example |
|---|---|---|
| Ranking | `Top N <Superlative> in <Competition/Scope> <Timeframe>` | `Top 10 Greatest Saves in Champions League History` |
| Story | `The <Person/Club> Who <Twist>` | `The Club That Went Bankrupt One Year After Winning` |
| Football Myths | `Myth: <Widely Believed Claim>` | `Myth: The Football Was Invented in England` |
| Failed Moments | `Top N <Fail Type> in Football History` | `Top 5 Worst Misses in Football History` |
...
| Workbook row | Rendered file |
|---|---|
| `SN 1`, Ranking | `SN001_ranking_top10-world-cup-goals.mp4` |
| `SN 2`, Football Myths | `SN002_myth_ball-invented-in-england.mp4` |
| `SN 3`, Failed Moments | `SN003_fails_worst-misses-in-history.mp4` |
...
2. Go to the first empty row and put the next number in **Video SN** (drag the last number down to auto-increment).
...
The rows below are the starter examples shipped in the workbook — delete them once you add real videos.
...
|---|---|---|---|---|---|---|
| 1 | Ranking | Top 10 Best Goals in World Cup History | 2026-09-20 | Created - Not Uploaded | — | YouTube Shorts |
| 2 | Football Myths | Myth: The Football Was Invented in England | 2026-09-18 | Scheduled | 2026-09-22 | YouTube Shorts |
| 3 | Failed Moments | Top 5 Worst Misses in Football History | 2026-09-15 | Posted | 2026-09-16 | YouTube Shorts |

---

## ⚠️ Ground rules

- **Never commit this folder** — `.gitignore` already excludes `Tracker/`; if you ever see it staged, run
  `git check-ignore -v Tracker/Video_Tracker.xlsx` to confirm.
